#!/usr/bin/env python3
"""
Deletes all runZero assets that match the inventory query: (type:mobile OR type:phone)

- Only deletes asset IDs returned by search=(type:mobile OR type:phone).
- Extra safety: also verifies each returned record has type == "Mobile" or "Phone" before deleting. So that it doesn't delete IP Phones that would normally be returned by type:phone
...
TOKEN = "OT603EDA2736014C4A7B38D118ADD9" # Get this API key by going into Organisations > Settings on your organisation > Generate Organisation API Key
ORG_ID = ""  # optional: org UUID (only needed for account-scoped tokens, leave blank if using an ORGANISATION TOKEN 


# Delete both Mobile and Phone assets
MOBILE_PHONE_QUERY = "(type:mobile OR type:phone)"
ALLOWED_TYPES = {"mobile", "phone"}
    ...
    s.headers.update(
        {
            ...
            "User-Agent": "runzero-delete-mobile-phone/1.0",
        }
    )
    return s
...
def fetch_mobile_phone_asset_ids(session: requests.Session) -> List[str]:
    url = f"{BASE_URL}/export/org/assets.jsonl"
    params: Dict[str, str] = {"search": MOBILE_PHONE_QUERY, "fields": "id,type"}
    if ORG_ID:
        params["_oid"] = ORG_ID
    ...
    for obj in iter_assets_jsonl(resp):
        ...
        # Only delete assets whose returned type is exactly Mobile or Phone
...
def main() -> int:
    ...
    print(f"Deleting assets matching query: {MOBILE_PHONE_QUERY}")

    print("Fetching matching assets via export JSONL...")
    ids = fetch_mobile_phone_asset_ids(session)

    if not ids:
        print("No Mobile or Phone assets found. Nothing to delete.")
        return 0

    print(f"Found {len(ids)} assets (Mobile or Phone). Starting deletion...")
